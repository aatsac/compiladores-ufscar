...
Este repositório contém a implementação incremental de um compilador completo para a **Linguagem Algorítmica (LA)**, desenvolvida pelo prof. Jander Moreira no DC/UFSCar.

O compilador foi construído em cinco trabalhos, cada um adicionando uma nova fase ao pipeline:
...
> O **ANTLR4** (usado nos trabalhos T2 a T5) é baixado automaticamente pelo Maven.
...
Código LA
    ...
    ▼
┌─────────────────┐
│ Análise Léxica  │  → tokens / erro léxico
└────────┬────────┘
         │
    ▼
┌──────────────────────┐
│ Análise Sintática    │  → árvore sintática / erro sintático
└──────────┬───────────┘
           │
      ▼
┌──────────────────────┐
│ Análise Semântica    │  → verificação de tipos e escopos / erros semânticos
└──────────┬───────────┘
           │
      ▼
┌──────────────────────┐
│  Geração de Código   │  → código C compilável
└──────────────────────┘
```
...
**Saída do T2, T3, T4 (sem erros):**
```
Fim da compilacao
```

**Saída do T5 (código C gerado):**
```c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
- **Java 11+**
- **ANTLR 4.13.1** — gerador de parsers (T2 a T5)
- **Maven 3.6+** — build e gerenciamento de dependências
- **GCC** — compilação do código C gerado (T5)
